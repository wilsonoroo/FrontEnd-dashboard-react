...
export default function DocumentosViewV1(props: { titulo: string }) {
  ...
  const [documentos, setDocumentos] = useState<DocumentoVaku[]>([]);
  ...
  if (isVersionRealtime) {
    divisionRepository = new FirebaseRealtimeRepository<DocumentoVaku>(
      `empresas/${idEmpresa}/documentos_por_mes`
    );
  }
  ...
  const calculateEstadisticas = useCallback(() => {
    const estadistic = documentos.reduce(
    ...
    const countByTipo = documentos.reduce(
      (
        acc: {
          [key: string]: {
            cantidad: number;
        ...
        const { checklist } = obj;
        const tipoKey: string = checklist.tipo;
  ...
  const getDocumentos = async () => {
    setLoadingData(true);
    let mes = moment().format("YYYY_MM");
    let mesAnterior = moment().subtract(1, "months").format("YYYY_MM");
    let mesAnteriro = await divisionRepository.getAllById(
      mesAnterior,
      DocumentoVaku
    );
    let mesActual = await divisionRepository.getAllById(mes, DocumentoVaku);
    console.log(
      "🚀 ~ file: DocumentosViewV1.tsx:115 ~ useEffect ~ mesAnteriro:",
      mesAnteriro
    );
    setDocumentos([...mesActual, ...mesAnteriro]);
